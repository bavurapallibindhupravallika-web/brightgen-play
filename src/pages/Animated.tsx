...
import { Palette, Sparkles, Loader2, Brain, PenLine, Download, FileText } from "lucide-react";
...
const Animated = () => {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${topic || "animated"}-script.txt`;
    a.click();
            ...
            <div className="grid grid-cols-5 gap-2">
              {storyTypes.map((type) => (
                <button key={type} onClick={() => setStoryType(type)}
                  className={`text-[10px] font-semibold py-1.5 px-1 rounded-lg transition-all ${storyType === type ? "bg-primary text-primary-foreground neon-glow" : "glass text-muted-foreground hover:text-foreground"}`}>
                  {type}
                </button>
              ))}
            </div>
